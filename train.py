...
upsampled_logits_shape = tf.stack([
                                  ...
                                  img_shape[1],#downsampled_logits_shape[1]*unsample_factor
                                  img_shape[2],#downsampled_logits_shape[2]*unsample_factor
...
upsampled_logits_pool4 = upsampled_logits + aux_logits_16s#pool4分类的输出与上采样结果进行相加
...
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_16s_pool3 = slim.conv2d(pool3_feature, number_of_classes,[1,1],
                                       ...
                                       weights_initializer=tf.zeros_initializer,
                                       scope='conv_pool3')
#Perform the second upsampling

upsample_filter_np_x2 = bilinear_upsample_weights(2,  # upsample_factor,
                                                  number_of_classes)

upsample_filter_tensor_x2 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/t_conv_x2')
#将第一次上采样结果upsampled_logits_pool4进行一个2倍上采样,输出shape跟aux_logits_16s_pool3一样
upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits_pool4, upsample_filter_tensor_x2,
                                          output_shape=tf.shape(aux_logits_16s_pool3),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')
#pool3分类后的输出与第二次上采样进行相加
...
def perform_crf(image, probabilities):
    ...
    image = image.squeeze()
    softmax = probabilities.squeeze().transpose((2, 0, 1))#调整形态

    # The input should be the negative of the logarithm of probability values
    # Look up the definition of the softmax_to_unary for more information
    unary = unary_from_softmax(softmax)#向量化

    # The inputs should be C-continious -- we are using Cython wrapper
    unary = np.ascontiguousarray(unary)#内存形态调整

    d = dcrf.DenseCRF(image.shape[0] * image.shape[1], number_of_classes)

    d.setUnaryEnergy(unary)#1d 函数
    ...
    d.addPairwiseEnergy(feats, compat=3,
                        ...
                        normalization=dcrf.NORMALIZE_SYMMETRIC)#计算一元次函数
    ...
    d.addPairwiseEnergy(feats, compat=10,
                        kernel=dcrf.DIAG_KERNEL,
                        normalization=dcrf.NORMALIZE_SYMMETRIC)#计算二元次函数
    Q = d.inference(5)
